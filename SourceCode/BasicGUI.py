...
class MultiThread(QtCore.QThread):
    ...
    def __init__(self, num, dynamicplot,currentData,currentLabel,para_C, Tol, MaxIter,KernelType,parent=None):  
    ...
    def run(self): #Overwrite run() method, put what you want the thread do here  
        ...
        self.trigger.emit("w1:"+QtCore.QString("%1").arg(float(a[0]))+"\r\n"+\
        ...
        self.trigger.emit("Total Data Number:"+QtCore.QString("%1").arg(DataNum)+"\r\n")
        self.trigger.emit("Accuracy:"+QtCore.QString("%1").arg(accuracy)+"\r\n")
        self.thread_stop = True
        return
#采用多线程实现混合高斯EM防止界面假死
class MultiThreadGaussian(QtCore.QThread):
    trigger = QtCore.pyqtSignal(str) # trigger传输的内容是字符串
    def __init__(self, num, dynamicplot,currentData,K,MaxIter,parent=None):  
        super(MultiThreadGaussian, self).__init__(parent)       
        self.thread_num = copy.deepcopy(num)
    ...
    def run(self): #Overwrite run() method, put what you want the thread do here  
        ...
        for pi_index in range(pi_size):
            self.trigger.emit("pi"+ QtCore.QString("%1").arg(pi_index) + ":"+QtCore.QString("%1").arg(float(pi_new[0,pi_index]))+"\r\n")
        D,K=numpy.shape(UK_new)
        #print UK_new
        for Kiter in range(K):
            self.trigger.emit("Uk"+ QtCore.QString("%1").arg(Kiter) + ": ")
            for Diter in range(D):
                self.trigger.emit(QtCore.QString("%1").arg(float(UK_new[Diter,Kiter])) + "     ")
            self.trigger.emit("\r\n")
        #print List_cov
        self.trigger.emit("\r\n")
        for Liter in range(len(List_cov)):
            ...
            if D1==D2:
                self.trigger.emit("Cov_k"+ QtCore.QString("%1").arg(Liter) + ": ")
                for D1iter in range(D1):
                    for D2iter in range(D2):
                        self.trigger.emit( QtCore.QString("%1").arg(float(List_cov[Liter][D1iter,D2iter])) + "     ")
                    self.trigger.emit("\r\n"+ "        ")
                self.trigger.emit("\r\n")
...
class Dialog(QtGui.QMainWindow):
    ...
    def CSVM(self):
        ...
        self.threads[self.threadcount]=MultiThread(self.threadcount, self.dynamicplot,self.currentData,self.currentLabel,para_C, Tol, MaxIter,KernelType)
        #b,alphas,w=OriginSVM.smoPlatt(self.currentData,self.currentLabel,para_C, Tol, MaxIter,KernelType)
        self.threads[self.threadcount].trigger.connect(self.update_text)  # connect to it's signal
        self.threads[self.threadcount].start()
        self.threadcount +=1
        return
    #?????????????????????????????????
    def EmMixGaussian(self):
        ...
        self.threads[self.threadcount]=MultiThreadGaussian(self.threadcount,self.dynamicplot,self.currentData, 2, 300)
        self.threads[self.threadcount].trigger.connect(self.update_text)
        self.threads[self.threadcount].start()
        self.threadcount +=1
        return
